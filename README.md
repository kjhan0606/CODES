...
specified terrestrial observer. CODES also propagates the first apparition
state over the chart interval with the DE441 long-term kernel. The JPL
topocentric track and the CODES geocentric track are plotted separately. The
output table records their angular separation together with ICRS and apparent
coordinates, azimuth, elevation, solar elongation, distance, IAU
constellation, and the numeric Horizons apparition record. Use `--jpl-only`
to omit the local DE441 calculation.
